...
class Migration(migrations.Migration):
    """Migracion para devolver nombre y apellido si nick esta nulo."""
    dependencies = [
        ('api', '0041_auto_20180530_1857'),
    ]
  ...
  BEGIN DECLARE v_id_specialist INT;
    ...
    ELSEIF flag = 2
      THEN SELECT
             max(m.id),
             (select count(id) from api_query where q.client_id=p_user_id and status=1 and category_id=p_aux_1)
           INTO v_id, v_total
           FROM api_message m
             JOIN api_query q ON m.query_id = q.id
